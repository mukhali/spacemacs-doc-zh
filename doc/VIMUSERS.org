...
该文档主要是用来补充=Documentation=文件中关于=vim=和=emacs=叙述的不足。有可能该文档
和=Documentation=文件有重合的地方，建议你两份文档都读，这样你就不会有遗漏了。
...
#+begin_src emacs-lisp
  ...
  ;; This relies on the c-c++ layer being enableds.
...
#+BEGIN_EXAMPLE
    ...
    [] = directory *not created by the command
#+END_EXAMPLE
