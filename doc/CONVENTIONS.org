...
函数应该遵循一下约定：
  - =spacemacs/xxx= 可交互函数 =xxx=
  - =spacemacs//xxx= 私有函数 =xxx= (implementation details)
  - =spacemacs|xxx= 是宏 =xxx=

变量遵循以下约定：
  - =spacemacs-xxx= 普通变量
  - =spacemacs--xxx= 私有变量(implementation details)

*** All layers
一个包在函数 =<layer>/init-xxx= 中完成初始化，其中：
  - =<layer>= layer的名字
  - =xxx= 包名
...
*** Reserved prefix
**** User prefix
~SPC o~ 任何layer都不能使用，自能保留给用户自由定制。

**** Major mode prefix
~SPC m~ 保留为当前major mode使用。三个按键也可以(比如。 ~SPC m h d~) ，
因为 ~SPC m~ = ~,~。
